//+------------------------------------------------------------------+
//|                                SentraPartners_MT5_DEBUG.mq5       |
//|                                   Sentra Partners Trading SaaS   |
//|                                                   Version 4.1    |
//|                                     COM LOGS COMPLETOS DEBUG     |
//+------------------------------------------------------------------+
...
{
   Print("===========================================");
   Print("Sentra Partners - API DEBUG MT5 v4.1");
   Print("Sistema com LOGS COMPLETOS para debug");
   Print("===========================================");
...
bool SendCompleteHeartbeat()
{
   ...
   Print("DEBUG: Posições totais = ", PositionsTotal());
   
   // POSIÇÕES ABERTAS com TODOS os dados
   string positionsJson = BuildPositionsArray();
   Print("DEBUG: Positions JSON = ", positionsJson);
   json += "\"positions\":" + positionsJson + ",";
   
   // HISTÓRICO COMPLETO
   string historyJson = BuildHistoryArray();
   Print("DEBUG: History JSON (primeiros 500 chars) = ", StringSubstr(historyJson, 0, 500));
   json += "\"history\":" + historyJson;
   
   json += "}";
   
   Print("DEBUG: JSON COMPLETO (primeiros 1000 chars):");
   Print(StringSubstr(json, 0, 1000));
   Print("========== FIM DEBUG JSON ==========");
   ...
   {
      Print("✗ Erro ao enviar heartbeat - Status: ", statusCode);
      return false;
   }
}
...
string BuildPositionsArray()
{
   ...
   Print("DEBUG BuildPositionsArray: Total de posições = ", total);
   
   for(int i = 0; i < total; i++)
   {
      ulong ticket = PositionGetTicket(i);
      Print("DEBUG BuildPositionsArray: Processando posição ", i, ", ticket = ", ticket);
      ...
      if(ticket > 0)
      {
         ...
         Print("DEBUG: Posição adicionada - Ticket: ", ticket, ", Symbol: ", PositionGetString(POSITION_SYMBOL));
      }
   }
...
string BuildHistoryArray()
{
   ...
   Print("DEBUG BuildHistoryArray: Buscando de ", TimeToString(from), " até ", TimeToString(to));
   
   if(!HistorySelect(from, to))
   {
      Print("DEBUG BuildHistoryArray: HistorySelect FALHOU!");
      return "[]";
   }
   
   int total = HistoryDealsTotal();
   Print("DEBUG BuildHistoryArray: Total de deals = ", total);
   
   if(total == 0) return "[]";
   
   // Limitar a 1000 deals para não sobrecarregar
   int maxDeals = MathMin(total, 1000);
   Print("DEBUG BuildHistoryArray: Processando até ", maxDeals, " deals");
   ...
   Print("DEBUG BuildHistoryArray: Deals processados (IN/OUT) = ", dealsProcessed);
...
int SendPostRequest(string url, string json)
{
   ...
   Print("DEBUG SendPostRequest: Enviando ", StringLen(json), " bytes para ", url);
   ...
   else
   {
      ...
      Print("✓ Status: ", statusCode, " | Response: ", response);
   }
