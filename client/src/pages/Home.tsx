    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Drawdown do Mês</CardTitle>
              <TrendingDown className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className={`text-2xl font-bold ${
                (dashboardData?.summary?.monthlyDrawdown || 0) > 5 ? "text-red-500" : "text-green-500"
              }`}>
                {(dashboardData?.summary?.monthlyDrawdown || 0).toFixed(2)}%
              </div>
              <p className="text-xs text-muted-foreground">
                Retração máxima mensal
              </p>
            </CardContent>
          </Card>
        </div>
