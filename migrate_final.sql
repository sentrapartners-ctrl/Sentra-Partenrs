-- Migração final - Parte 1: Estrutura users
ALTER TABLE users MODIFY COLUMN role ENUM('client', 'manager', 'admin') NOT NULL DEFAULT 'client';
ALTER TABLE users ADD COLUMN walletAddress VARCHAR(128) UNIQUE AFTER password;
ALTER TABLE users ADD COLUMN authMethod ENUM('email', 'wallet', 'both') NOT NULL DEFAULT 'email' AFTER walletAddress;
ALTER TABLE users ADD COLUMN managerId INT AFTER authMethod;
...
-- Parte 3: Atualizar trading_accounts
ALTER TABLE trading_accounts 
  MODIFY COLUMN platform ENUM('MT4', 'MT5', 'cTrader', 'DXTrade', 'TradeLocker', 'MatchTrade', 'Tradovate') NOT NULL;
ALTER TABLE trading_accounts ADD COLUMN classification VARCHAR(128) AFTER lastHeartbeat;
ALTER TABLE trading_accounts ADD COLUMN isCentAccount BOOLEAN NOT NULL DEFAULT FALSE AFTER classification;

-- Parte 4: Atualizar trades
ALTER TABLE trades ADD COLUMN origin ENUM('robot', 'manual', 'unknown') NOT NULL DEFAULT 'unknown' AFTER comment;

-- Parte 5: Inserir dados padrão
