    ...
    for (const pos of positions) {
      try {
        await createOrUpdateTrade({
          ...
          stopLoss: null,
          takeProfit: null,
          profit: Math.round(parseFloat(pos.profit) * 100),
          commission: pos.commission ? Math.round(parseFloat(pos.commission) * 100) : 0,
          swap: pos.swap ? Math.round(parseFloat(pos.swap) * 100) : 0,
          comment: "",
          magicNumber: 0,
        });
        saved++;
    ...
    for (const trade of trades) {
      try {
        await createOrUpdateTrade({
          ...
          stopLoss: null,
          takeProfit: null,
          profit: Math.round(parseFloat(trade.profit) * 100),
          commission: trade.commission ? Math.round(parseFloat(trade.commission) * 100) : 0,
          swap: trade.swap ? Math.round(parseFloat(trade.swap) * 100) : 0,
          comment: "",
          magicNumber: 0,
        });
        saved++;
