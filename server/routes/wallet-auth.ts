 ...
 * Cria ou busca usuário no banco de dados e vincula a wallet
 */
router.post("/wallet-login", async (req, res) => {
  try {
    ...
    if (user.length === 0) {
      ...
      console.log("[Wallet Login] Usuário encontrado:", user[0].id);
    }

    // Criar sessão de wallet no banco
    ...
    if (req.session) {
      ...
      req.session.save();
      console.log("[Wallet Login] Sessão criada para usuário:", user[0].id);
    }
